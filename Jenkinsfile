...
environment {
    ...
    stage('Deploy to EC2 Instance') {
        steps {
            ...
            bat '''
                "C:\\Program Files\\Git\\bin\\bash.exe" -c "ssh -i '%KEY_PATH%' -o StrictHostKeyChecking=no %EC2_USER%@%EC2_HOST% 'export AWS_ACCESS_KEY_ID=%AWS_ACCESS_KEY_ID% && export AWS_SECRET_ACCESS_KEY=%AWS_SECRET_ACCESS_KEY% && export AWS_DEFAULT_REGION=%AWS_REGION% && ./deploy.sh %IMAGE_TAG% \\"%MONGO_URI%\\?retryWrites=true^&w=majority"' "
            '''
            }
        }
    }
