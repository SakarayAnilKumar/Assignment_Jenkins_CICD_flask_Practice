...
environment {
    ...
    stages {
        ...
        stage('Validate Application Health') {
            steps {
                bat '''
                    "C:\\Program Files\\Git\\bin\\bash.exe" -c "ssh -i '%KEY_PATH%' -o StrictHostKeyChecking=no %EC2_USER%@%EC2_HOST% 'sleep 5 && curl -f http://localhost:5000/ > /dev/null && echo App is healthy!'"
                '''
            }
        }
}
